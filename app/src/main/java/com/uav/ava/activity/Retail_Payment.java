...
public class Retail_Payment extends AppCompatActivity implements TransactionResult {
    ...
    private final int REQUEST_CODE_UPI_TXN = 10010;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_retail__payment);
        this.context = this;
        ...
        try {
           ...
           // flightJson = new JSONObject(sharedPreferences.getString(ApplicationConstant.CACHE_PASSENGER_DETAIL, null)) ;
            ...
            Utility.exceptionAlertDialog(Retail_Payment.this,"Alert!","Something went wrong, Please try again!","Report",Utility.getStackTrace(e));
        }
        ...
        try {
            ...
            Timestamp ts = new Timestamp(((Date)df.parse(CLPJson.getString("saledate"))).getTime());
            Date date = new Date();
            date.setTime(ts.getTime());
            posSaleVO.setInvoiceDate(new SimpleDateFormat("yyyy-MM-dd").format(date));

            posSaleVO.setEcomSalesDate(new SimpleDateFormat("yyyy-MM-dd").format(date));
            ...
            posSaleVO.setPassengerName(CLPJson.getString("customername"));
            ...
            posSaleVO.setOrderNumber(CLPJson.getString("roomno"));
       ...
       /* applycoupon.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                VolleyUtils.makeJsonObjectRequest(Retail_Payment.this, ProductBO.applyAviationCoupon(posSaleVO.getContactNumber(),coupon.getText().toString(),netamount.getText().toString(),posSaleVO.getPrincipalID()) , new VolleyResponseListener() {
                    @Override
                    ...
                    public void onResponse(Object resp)  {
                        ...
                        try {
                            if(response.getString("status").equals("fail")){
                                ...
                                VolleyUtils.furnishErrorMsg(  "Error " ,response, Retail_Payment.this);
                            }else {
        ...
        getoffer.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(promoHash!=null && PromotionId!=null && Productqty!=0){
                    VolleyUtils.makeJsonObjectRequest(Retail_Payment.this, OfferBO.getOfferByCard(PromotionId,promoHash,Productqty,grosstotaltext.getText().toString()), new VolleyResponseListener() {
                        @Override
                        public void onError(String message) {
                        }
        ...
        saveaviation.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(Integer.parseInt(posSaleVO.getPaymentTypeID())== PaymentTypeVO.MOSAMBEE ||  Integer.parseInt(posSaleVO.getPaymentTypeID())== PaymentTypeVO.EZETAP  ||  Integer.parseInt(posSaleVO.getPaymentTypeID())== PaymentTypeVO.CASH  ||  Integer.parseInt(posSaleVO.getPaymentTypeID())== PaymentTypeVO.UPI){
                    getUniqueTxnId();
                }else{
                    doSaveInvoice(null);
                }
        ...
        paymenttype.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if(MotionEvent.ACTION_UP == event.getAction()) {
                    ...
                    connectionVO.setSharedPreferenceKey(ApplicationConstant.CACHE_PAYMENT_TYPE_With_Action);
    ...
    public void setPaymenttypeByActionType(){

        try{
            String TypeAction =getIntent().getStringExtra("TypeAction");
            JSONObject newPaymentTypeCache=new JSONObject(sharedPreferences.getString(ApplicationConstant.CACHE_PAYMENT_TYPE,null));


            JSONArray jsonArray =newPaymentTypeCache.getJSONArray("data");
            JSONArray paymenttypearray =new JSONArray();
            for(int i=0;i<jsonArray.length();i++){
                JSONObject jsonObject =jsonArray.getJSONObject(i);
                if(TypeAction.equals("Railway")){
                    paymenttypearray.put(jsonObject);
                }else {
                    if(Integer.parseInt(jsonObject.getString("paymentTypeID"))==PaymentTypeVO.UPI){
                        continue;
                    }
                    paymenttypearray.put(jsonObject);
                }
            ...
            jsonObject.put("data",paymenttypearray);

            SharedPreferences.Editor edit= sharedPreferences.edit();
            edit.putString( ApplicationConstant.CACHE_PAYMENT_TYPE_With_Action,jsonObject.toString() );
            edit.apply();
            edit.commit();

            Log.w("sdfsfds",sharedPreferences.getString(ApplicationConstant.CACHE_PAYMENT_TYPE_With_Action,null));
    ...
    private boolean validatePaymentType(){
        ...
        if(!paymenttypeid.getText().toString().equals("")){
            ...
            if( netAmt<0){
                Utility.alertDialog(Retail_Payment.this,"Validation!","Spl. discount is greater than the gross amount","OK");
                valid= false;
            }
        }
    ...
    private void getUniqueTxnId(){
        VolleyUtils.makeJsonObjectRequest(Retail_Payment.this, AviationSaleBO.getUniqeTxnId(), new VolleyResponseListener() {
            @Override
            public void onError(String message) {
            }
            ...
            public void onResponse(Object resp) throws JSONException {
                ...
                if(response.getString("status").equalsIgnoreCase("SUCCESS")){
                    ...
                    Utility.alertDialog(Retail_Payment.this,"Validation!","Please try again","OK");
                }
...
*/
    ...
    private void doSaveInvoice(String ccResp){
        ConnectionVO connectionVO = AviationSaleBO.saveRetailSale();
        try {
        ...
        VolleyUtils.makeJsonObjectRequest(context,connectionVO , new VolleyResponseListener() {
            ...
            public void onResponse(Object resp) throws JSONException {
                ...
                if(response.has("status")){
                    ...
                    Intent intent =new Intent(Retail_Payment.this,Success.class);
        ...
        try {
            switch (action) {
                ...
                case EZETAP_SALE_TXN_PREPARE_ACTION:
                    ...
                    }else if(Integer.parseInt(posSaleVO.getPaymentTypeID())==PaymentTypeVO.CASH) {
                        ezeTapCashPrepareAction(resultCode, intent);
                    }else if(Integer.parseInt(posSaleVO.getPaymentTypeID())==PaymentTypeVO.UPI) {
                        ezeTapUPIPrepareAction(resultCode, intent);
                    }
                    break;
                case EZETAP_SALE_TXN_ACTION:
                    if(Integer.parseInt(posSaleVO.getPaymentTypeID())==PaymentTypeVO.EZETAP) {
                        ...
                        ezeTapUPIAction(resultCode, intent);
                    }
                    break;
    ...
    private void ezeTapUPIPrepareAction(int resultCode, Intent intent){
        if (resultCode == RESULT_OK) {
            JSONObject jsonRequest = new JSONObject();
            try {
    ...
    private void ezeTapUPIAction(int resultCode, Intent intent){
        JSONObject jsonRequest = new JSONObject();
        try {
            if (resultCode == RESULT_OK) {
                JSONObject response = new JSONObject(intent.getStringExtra("response"));
